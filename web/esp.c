...
void setup() {
  ...
  // Seed random for temperature simulation
  randomSeed(micros());
  ...
  Serial.println();
  Serial.println("=== ESP32 pH Sensor Reading (Pre-calibrated) ===");
...
void loop() {
  ...
  if (millis() - lastRead >= 1000) {
    ...
    // Publish the data over MQTT (now includes temperature)
    publishSensorData();
  }

  // Simulate temperature updates (runs independently, no separate publish)
  simulateTemperature();
}
...
// =========================
// Print Serial Output (now includes temperature in same line)
// =========================
void printSensorData() {
  ...
  Serial.println(temperature, 2); // newline here so all shown together
}
      ...
      <script>
        function updateData() {
          ...
          .then(data => {
            ...
            // if you want to display temperature on the web UI later, you can add an element and update it here
          });
        }
...
void handleData() {
  String json = "{\"tds\":" + String(tdsValue, 2) + ",\"voltage\":" + String(voltage, 3) + ",\"ph\":" +
                String(lastPH, 2) + ",\"ph_voltage\":" + String(lastVoltage, 3) + ",\"temperature\":" + String(temperature, 2) + "}";
...
// Publish sensor data to MQTT topic (now includes temperature)
...
// - updates the global temperature variable
// - prints temperature to Serial
// - DOES NOT publish to a separate topic (avoids topic mismatch)
// =========================
void simulateTemperature() {
  static float currentTemp = 25.0;
  static unsigned long lastTempUpdate = 0;

  // Update roughly every 1.5 seconds
  if (millis() - lastTempUpdate >= 1500) {
    lastTempUpdate = millis();

    // Small drift: random value between -0.15 and +0.15
    float variation = ((float)random(-15, 16)) / 100.0;
    currentTemp += variation;

    // Clamp to 24.0 - 28.0 range
    if (currentTemp < 24.0) currentTemp = 24.0;
    if (currentTemp > 28.0) currentTemp = 28.0;

    // Update the global temperature used by TDS compensation
    temperature = currentTemp;

    // Print simulated temperature (kept for debugging)
    //Serial.print("Simulated Temperature updated to: ");
